# launcher.ps1 - THE ONE SYSTEM v3.1 (Fully self-contained, no downloads needed)
...
cls
color 07
title  THE ONE WINDOWS AUTHORIZED v3.1
...
# ============================================================
#  内嵌的 Ohook 激活脚本（Office）- 已修改为 THE ONE
# ============================================================
$ohookScript = @'
@set masver=3.12
@echo off
...
cls
color 07
title  THE ONE OFFICE AUTHORIZED v3.1
...
echo:
echo                 [1] Install THE ONE Office Activation
echo:
echo                 [2] Uninstall THE ONE
...
call :dk_color %Blue% "Smart App Control may prevent you from opening Office after THE ONE activation."
...
title  Uninstall THE ONE Office Activation %masver%
...
echo:
echo Uninstalling THE ONE activation...
echo:
...
echo THE ONE activation is not installed.
) else (
if defined _unerror (
call :dk_color %Red% "Failed to uninstall THE ONE activation."
call :oh_checkapps
...
call :dk_color %Green% "Successfully uninstalled THE ONE activation."
)
)
echo __________________________________________________________________________________________
...
if exist "%_hookPath%\%%#" (set "ierror=Remove Previous THE ONE Install [%%#]")
)
...
if exist "%%~A" (set "ierror=Remove Previous THE ONE Install [%%#]")
)
)
)
...
if exist "%_hookPath%\sppcs.dll" (set "ierror=Remove Previous THE ONE mklink sppcs.dll")
...
if defined ierror (
set error=1
call :dk_color %Red% "Installing THE ONE                        [Failed to %ierror%]"
echo:
call :oh_checkapps
if defined checknames (
call :dk_color %Blue% "Close [!checknames!] and try again."
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------CQ-B----FQ-cgBh-G4-cwBs-GE-d-Bp-G8-bg------CQTkB---------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------
:sppc64.dll:
...
function Invoke-EmbeddedScript {
    ...
    try {
        ...
        [System.IO.File]::WriteAllText($tempPath, $ScriptContent, $utf8NoBom)
...
while ($true) {
    ...
    switch ($key) {
        '1' {
            Invoke-EmbeddedScript -ScriptContent $hwidScript -ScriptName "HWID_Activation.cmd" -CustomTitle "THE ONE WINDOWS AUTHORIZED v3.1"
        }
        '2' {
            Invoke-EmbeddedScript -ScriptContent $ohookScript -ScriptName "Ohook_Activation_AIO.cmd" -CustomTitle "THE ONE OFFICE AUTHORIZED v3.1"
        }
        '3' {
            Invoke-DeepClean
